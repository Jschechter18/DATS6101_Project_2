...
```
The bar plot of Pass/Fail distribution showed that the majority of students passed, as indicated by the taller blue bar representing pass status (1), compared to the shorter orange bar representing fail status (0). This indicates an imbalanced dataset, which is important to keep in mind because classification models may need balancing techniques like resampling or weighting to handle the imbalance.
...
We selected the following features based on their potential influence on academic performance:Attendance, Family Income Level, Extracurricular Activities and Internet Access at Home. These variables were used to train our model and understand patterns in student success and risk.
...
Pass Recall is much higher for Logistic Regression — about 90.56%, compared to 53.35% for Random Forest. This means Logistic Regression is better at identifying students who will pass. However, Fail Recall tells a different story — Random Forest outperforms Logistic Regression significantly, correctly identifying 98.42% of students who will fail, compared to only 44% by Logistic Regression. Looking at the AUC, Random Forest scores 0.896, which is excellent, while Logistic Regression scores 0.804, which is still good but not as strong.
...
**Analysis**
The random forest model is excellent at detecting students likely to fail, but poor at detecting passing students. The logistic regression model is excellent at detecting students likely to pass, but poor at detecting failing students.Which model to use is dependent on which the professor is more interested in catching. If the professor wants to predict which students are likely to fail, use the random forest. If the professor wants to predict which students will pass, use the logistic regression model.

**Conclusion**

Attendance is the strongest predictor of student success. As students attend more classes, their chances of passing increase. Professors should make it clear to students that attending class is very important. Although family income did not show a strong effect based on p-value, other methods suggested it may still have some link to passing or failing. This could point to hidden issues or unfair disadvantages and may need more study. 
Overall, both models used—Random Forest and Logistic Regression—worked well but serve different goals. Random Forest is better for spotting students who might fail, while Logistic Regression is good for finding those likely to pass. In the end, both models agree: attendance really matters.
