...
```{r}
students$Pass_Fail <- ifelse(students$Grade %in% c("A", "B", "C"), "Pass", "Fail")
...
students %>%
  ...
  group_by(Gender) %>%
  mutate(Proportion = n / sum(n)) %>%
  ggplot(aes(x = Gender, y = Proportion, fill = Grade)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Normalized Grade Distribution by Gender",
    x = "Gender",
    y = "Proportion"
  ) +
  ylim(0, 0.6) 
...
Do Attendance levels affect passing?
```{r}
students <- students %>%
  mutate(Attendance_Bin = cut(Attendance, breaks = c(0, 60, 70, 80, 90, 100), 
                              labels = c("<60%", "60–70%", "70–80%", "80–90%", "90–100%")))

ggplot(students, aes(x = Attendance_Bin, fill = Pass_Fail)) +
  geom_bar(position = "fill") +
  labs(title = "Pass/Fail Proportion by Attendance", x = "Attendance %", y = "Proportion") +
  theme_minimal()
